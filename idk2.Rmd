...
# ✅ NEW BLOCK: Use all updated FPCA features from nharm
fp_features <- grep("^FP[12]_", names(fpca_data), value = TRUE)

# Prepare modeling dataset: only target + FP scores
model_data <- fpca_data %>%
  dplyr::select(subject.identifier, all_of(fp_features))

# Check the features you're actually using
print(length(fp_features))       # Should be 40 if nharm = 20
print(colnames(model_data)[1:6]) # Spot check the first few
