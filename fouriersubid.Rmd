...
# Define Fourier basis parameters (match B-spline approach)
nbasis <- length(time)  
...
Lfdobj <- 4    
lambda <- 1e-2
fdParObj <- fdPar(fd(matrix(0, basis$nbasis, 1), basis), Lfdobj, lambda)
...
model_combined <- nnet:: multinom(subject.identifier ~ FP1.1 + FP1.2 + FP1.3 + FP1.4 + FP1.5 +
...
model <- nnet:: multinom(subject.identifier ~ FP1.1 + FP1.2 + FP1.3 + FP1.4 + FP1.5 + FP2.1 + FP2.2 + FP2.3 + FP2.4 + FP2.5 + FP2.6 + FP2.7 + FP2.8 + FP2.9, data = fp1modeldata)
...
train_indices <- createDataPartition(fp1modeldata$subject.identifier, p =0.75, list=F) 
...
```{r}
## Partition the data into 75% training and 25% testing based on subject.identifier
train_model_fp1 <- nnet:: multinom(train_condition ~ FP1.1 + FP1.2 + FP1.3 + FP1.4 + FP1.5, data = train_scores_df)
...
train_model_fp2 <- nnet:: multinom(train_condition ~ FP2.1 + FP2.2 + FP2.3 + FP2.4 + FP2.5, data = train_scores_df)
...
```{r}
train_model_combined <- nnet::multinom(subject.identifier ~ FP1.1 + FP1.2 + FP1.3 + FP1.4 + FP1.5 +
                                       FP2.1 + FP2.2 + FP2.3 + FP2.4 + FP2.5, 
...
rf_model <- randomForest(as.factor(subject.identifier) ~ FP1.1 + FP1.2 + FP1.3 + FP1.4 + FP1.5 +
                                                   FP2.1 + FP2.2 + FP2.3 + FP2.4 + FP2.5, 
